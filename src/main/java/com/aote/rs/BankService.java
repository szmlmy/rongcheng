			...
			for (int i = 0; i < array.length(); i++) {
				...
				//通过用户姓名，银行账号等信息查找用户详细信息
				 final String Sql = "select ISNULL(u.f_phone,'null') f_phone,isnull(u.f_bankname,'null') f_bankname,ISNULL( u.f_idofcard,'null') f_idofcard,isnull(u.f_userid,'null') f_userid,isnull(u.f_username,'null') f_username,isnull(u.f_address,'null') f_address,isnull(h.a,GETDATE()) a,isnull(h.f_inputtor,'null') f_inputtor,isnull(h.id,0) id  from t_userfiles u left join " +
						...
						" where f_username= '" +f_usermc + "' and f_idofcard='"+f_idofcard+"' and f_state='已抄表' and shifoujiaofei='否' " +
						" FOR XML PATH('')),1,1,'') as a from t_handplan) h " +
						" on u.f_username = h.f_username " +
						" where u.f_username= '" +f_usermc + "' and u.f_idofcard='"+f_idofcard+"' and h.f_state='已抄表' and h.shifoujiaofei='否' "; 
					...
					if(l.size() == 0) {
						...
						for (int j = 0; j < l.size(); j++) {
							...
							if (oughtfee == b_oughtfee) {
								...
								// 上次抄表底数
								Double lastinputgasnum = Double.parseDouble(((Object[]) l.get(j))[6]+"");
								// 本次抄表底数
								Double lastrecord = Double.parseDouble(((Object[]) l.get(j))[7]+"");
								// 上次抄表日期
								...
								final String Sql1 = "select id from t_sellinggas where f_userid ='" + userid +"' order by f_deliverydate desc";
								...
								// 取出未抄表记录以及用户信息
								Map<String, Object> handmap1 = (Map<String, Object>) sellList.get(0);
								String selId = handmap1.get("id").toString();
								// 更新抄表记录sellid
								String updateHandplan = "update t_handplan set f_sellid =" +selId + " where id in ("
									+ handIds + ")";
